...
export default function CourierOrderDetailPage() {
  ...
  const handleAcceptOrder = async () => {
    if (!confirm('Buyurtmani yetkazib berishga qabul qilish?')) return
  ...
  const handleStartDelivery = async () => {
    if (!confirm('Yetkazib berishni boshlash?')) return
  ...
  const handleCompleteDelivery = async () => {
    if (!confirm('Buyurtma yetkazib berildimi?')) return
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div className="flex justify-between items-start mb-6">
              ...
              <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                ...
                {order.status === 'delivered' ? 'Yetkazib berildi' :
                 ...
                 order.status === 'assigned_to_courier' ? 'Tayinlangan' :
                 'Tayyor'}
              </span>
            </div>
          ...
          <div className="bg-white rounded-lg shadow-lg p-6 space-y-3">
            ...
            {order.status === 'assigned_to_courier' && (
              ...
              >
                {updating ? 'Yangilanmoqda...' : 'Yetkazib berishni boshlash'}
              </button>
            )}
            {order.status === 'delivering' && (
              ...
              >
                {updating ? 'Yangilanmoqda...' : 'Buyurtma yetkazib berildi'}
              </button>
            )}
          </div>
        </div>
